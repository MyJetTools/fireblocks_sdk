...
impl CallbackType{
    pub async fn serialize(data: &str){
        ...
        let callback_type = value["type"].as_str().unwrap();

        match callback_type{
            callback_type if callback_type == "transaction_created" => {
                let data = FireblocksCallbackBaseData::deserialize(&value["data"].to_string()).await;
                let transaction_details = TransactionDetails::deserialize(&value["data"].to_string()).await;
                let callback_type = CallbackType::TransactionCreated(data, transaction_details);
