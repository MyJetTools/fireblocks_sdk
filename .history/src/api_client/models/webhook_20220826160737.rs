...
impl CallbackType{
    pub async fn serialize(data: &str){
        ...
        match callback_type.as_str(){
            ...
            "VAULT_ACCOUNT_ADDED" => {
                let data: TransactionDetails = serde_json::from_value(data_value.clone()).unwrap();
                CallbackType::VaultAccountAdded(fireblocks_base_data, data)
            },
            "VAULT_ACCOUNT_ASSET_ADDED" => CallbackType::VaultAccountAdded(fireblocks_base_data, data),
            "INTERNAL_WALLET_ASSET_ADDED" => CallbackType::TransactionApprovalStatusUpdate(fireblocks_base_data, data),
            "EXTERNAL_WALLET_ASSET_ADDED" => CallbackType::TransactionApprovalStatusUpdate(fireblocks_base_data, data),
            "EXCHANGE_ACCOUNT_ADDED" => CallbackType::TransactionApprovalStatusUpdate(fireblocks_base_data, data),
            "FIAT_ACCOUNT_ADDED" => CallbackType::TransactionApprovalStatusUpdate(fireblocks_base_data, data),
            "NETWORK_CONNECTION_ADDED" => CallbackType::TransactionApprovalStatusUpdate(fireblocks_base_data, data),
            _ => println!("0"),
